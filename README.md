...
| **🤖 架构师 AI 增强** | 自由模式下 AI 可从现有命令中选择，支持状态创建和跳转建议 |
...
| **自由模式 (free)** ⭐ | 架构师 AI 动态管理状态和命令集。支持从现有命令中选择、创建新状态（必须为自由模式）、提议状态跳转。深度限制最多 5 层。 | 探索性任务（如调研、创意） |

#### 🌟 自由模式工作流程（v1.3+）

```
用户输入 → 自由模式检测 → 架构师 AI 选择现有命令 → 
更新状态可用命令 → 执行者 AI 回复
```
...
| 能力 | 说明 | 是否允许 |
| :--- | :--- | :--- |
| **选择现有命令** | 从已注册的命令列表中选择 3-5 个相关命令 | ✅ **允许** |
| **更新状态描述** | 根据上下文动态更新当前状态描述 | ✅ **允许** |
| **创建新状态** | 可提议创建新状态（**必须是自由模式**） | ✅ **允许** |
| **状态跳转** | 可提议跳转到现有状态 | ✅ **允许** |
| **生成新命令** | 不允许，只能从现有命令中选择 | ❌ **禁止** |
...
# 对话测试
用户："你现在有什么可用的命令吗"
AI: "我可以使用以下命令：search_file, filter_result, open_file..."

# 状态跳转
用户："跳转到设置状态"
AI: "好的，正在跳转到 settings_state..."
```
...
- ✅ 超时机制自动清理返回值
- ✅ **架构师 AI 增强**：自由模式下 AI 可从现有命令中选择，支持状态创建和跳转建议
- ✅ **命令注册表完善**：添加 get_disclosed_commands 和 get_system_commands 方法
- ✅ **组件职责分离**：明确 CommandRegistry、StateMachine、SystemController 的职责边界

**🐛 修复：**
- ✅ 修复 state_jump 缺少 command_transitions 时不跳转的问题
- ✅ 优化处理器注册逻辑，支持显式配置优先 + 自动 fallback
- ✅ 改进类型提示和边界条件处理
- ✅ 修复 registry.py 缺失方法导致的运行时错误

**📝 文档：**
- ✅ 更新 README 说明处理器行为特征
- ✅ 添加返回值追踪使用指南
- ✅ 完善双层控制机制说明
- ✅ 添加自由模式工作流程和架构师 AI 能力矩阵
